...
func filterByLabels(s spec.Spec, issues service.Issues, merges service.Merges) (service.Issues, service.Merges) {
	...
	case spec.SelectionAll:
		...
		if len(s.Issues.IncludeLabels) > 0 {
			issues, _ = issues.Select(func(c service.Issue) bool {
	...
	case spec.SelectionAll:
		...
		if len(s.Merges.IncludeLabels) > 0 {
			merges, _ = merges.Select(func(c service.Merge) bool {
...
func resolveIssueMap(issues service.Issues, sortedTags service.Tags, futureTag service.Tag) issueMap {
	im := issueMap{}

	for _, i := range issues {
		// sortedTags are sorted from the most recent to the least recent
		tag, ok := sortedTags.Last(func(tag service.Tag) bool {
...
func resolveMergeMap(merges service.Merges, cm commitMap, futureTag service.Tag) mergeMap {
	mm := mergeMap{}
...
func toIssueGroup(title string, issues service.Issues) changelog.IssueGroup {
...
func toMergeGroup(title string, merges service.Merges) changelog.MergeGroup {
