...
func TestRepo_EnsurePermissions(t *testing.T) {
	tests := []struct {
		name          string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := r.EnsurePermissions(tc.ctx)
...
func TestRepo_FetchTags(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			ctx:          context.Background(),
			expectedTags: service.Tags{remoteTag},
		},
	}
...
func TestRepo_FetchParentCommits(t *testing.T) {
	tests := []struct {
		...
		ctx             context.Context
		ref             string
		expectedCommits service.Commits
		expectedError   string
	}{
		{
			name: "CommitFails",
		...
		},
		{
			...
			ctx:             context.Background(),
			ref:             "c3d0be41ecbe669545ee3e94d31ed9a4bc91ee3c",
			expectedCommits: service.Commits{remoteCommit2, remoteCommit1},
		},
	}
...
func TestRepo_FetchIssuesAndMerges(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			ctx:            context.Background(),
			since:          since,
			expectedIssues: service.Issues{remoteIssue},
			expectedMerges: service.Merges{remoteMerge},
		},
	}
